  ...
  function Output()
  {
	...
	if (!empty($uploadpk))
	  ...
	  if (empty($rc))
	    ...
	    $V .= PopupAlert('Debian-Lics added to job queue');
	    }
	  else
	    {
	    $V .= PopupAlert("Debian-Lics failed: $rc");
	    }
	  }

	/* Set default values */
	if (empty($GetURL)) { $GetURL='http://'; }
	/* Get upload_pk's for all debian uploads */
	...
	if (empty($Results[0]['upload_pk']))
	  ...
	  {
	  /* Loop thru results to obtain all pfiles in their uploadtree recs*/
          $Lics = array();
