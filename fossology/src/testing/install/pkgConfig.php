 ...
 *
 *  pinstall prepares a system for the installation of fossology packages.
 *
...
class ConfigSys {
  ...
  function __construct($osFlavor, $osVersion)
  {
    ...
    $releases = parse_ini_file(strtolower($osFlavor) . '.ini', 1);
    //echo "the parsed ini file is:\n";
    //print_r($releases) . "\n";
...
function copyFiles($files, $dest)
{
  ...
  {
    //echo "DB:CopyFiles: single file case, calling copy\n";
  ...
  switch ($osType) {
    ...
    case 'Debian':
      ...
      else
      {
        ...
        if(!copy('apache.conf', '/etc/fossology/apache.conf'))
        {
          echo "FATAL!, Cannot configure fossology into apache2\n";
          return(FALSE);
        }
        if(!symlink('/etc/fossology/apache.conf','/etc/apache2/conf.d/fossology'))
        {
...
function configDebian($osType, $osVersion)
{
  ...
  // @todo Take the callto parse_ini_file out, and reference the attributes from
  // ConfigSys!

  $releases = parse_ini_file(strtolower($osType) . '.ini', 1);
  //echo "the parsed debian ini file is:\n";
  //print_r($releases) . "\n";

  foreach($releases as $release)
  {
    //echo "release is:\n";print_r($release) . "\n";
    switch ($release['osversion']) {
      case 5.0:
        ;
        break;
      case 6.0:
        ...
        */
        $psqlFiles = array(
          'debian/6/pg_hba.conf',
          'debian/6/postgresql.conf');
        try
        {
        ...
        try
        {
          copyFiles('debian/6/cli-php.ini', '/etc/php5/cli/php.ini');
        } catch (Exception $e)
        {
        ...
        try
        {
          copyFiles('debian/6/apache2-php.ini', '/etc/php5/apache2/php.ini');
        } catch (Exception $e)
        {
  ...
  {
    echo "Erorr! Could not restart postgresql, please restart by hand\n";
    return(FALSE);
  }
  return(TRUE);
}  // configDebian
