...
class cliParamsTest4Ununpack extends PHPUnit_Framework_TestCase {
  ...
  protected function setUp() {
    ...
    if(file_exists($this->UNUNPACK_PATH))
    {
      $usage = 'Usage: ../../agent/ununpack [options] file [file [file...]]';
    }
    else
    {
      $this->assertFileExists(($this->UNUNPACK_PATH),
      $message = 'FATAL: cannot find executable file, stop testing\n');
    }

    // run it
    $last = exec("$this->UNUNPACK_PATH -h 2>&1", $out, $rtn);
  ...
  function testNormalIso1(){
    ...
    $command = "$this->UNUNPACK_PATH -qCR $TEST_DATA_PATH/523.iso -d $TEST_RESULT_PATH";
    exec($command);
  ...
  function testNormalIso2(){
    global $TEST_DATA_PATH;
    global $TEST_RESULT_PATH;
    $command = "$this->UNUNPACK_PATH -qCR $TEST_DATA_PATH/imagefile.iso -d $TEST_RESULT_PATH";
    exec($command);
  ...
  function testNormalMultyProcess(){
    global $TEST_DATA_PATH;
    global $TEST_RESULT_PATH;
    $command = "$this->UNUNPACK_PATH -qCR -m 10 $TEST_DATA_PATH/".
