...
int	main	(int argc, char *argv[])
{
  ...
  if (getuid() == 0)
    ...
    if (!KillSchedulers)
      ...
      if (!G)
	{
	fprintf(stderr,"ERROR: Group '%s' not found.  Aborting.\n",PROJECTGROUP);
	DBclose(DB);
	exit(-1);
	}
	...
	exit(-1);
      ...
      if (!P)
	{
	fprintf(stderr,"ERROR: User '%s' not found.  Will not run as root.  Aborting.\n",PROJECTUSER);
	DBclose(DB);
	exit(-1);
	}
      if ((setuid(P->pw_uid) != 0) || (seteuid(P->pw_uid) != 0))
	{
	fprintf(stderr,"ERROR: Cannot run as user '%s'.  Will not run as root.  Aborting.\n",PROJECTUSER);
	DBclose(DB);
	exit(-1);
	}
      } /* if !!KillScheduler */
    ...
    if (!Match)
      {
      fprintf(stderr,"ERROR: You are not in group '%s'.  Aborting.\n",PROJECTGROUP);
      DBclose(DB);
      exit(-1);
      }
    } /* check group access */
  ...
  if (!KillSchedulers && LogFile)
    ...
    if (freopen(LogFile,"wb",stdout) == NULL)
      {
      fprintf(stderr,"ERROR: Unable to write to logfile '%s'\n",LogFile);
      DBclose(DB);
      exit(-1);
      }
    if ((dup2(fileno(stdout),fileno(stderr))) < 0)
      {
      fprintf(stderr,"ERROR: Unable to write to redirect stderr to log\n");
      DBclose(DB);
      exit(-1);
      }
    }
  ...
  if (ResetQueue)
	...
	fprintf(stderr,"Job queue reset.\n");
	}
  ...
  if (Test)
    ...
    if (rc == 0) fprintf(stderr,"STATUS: All scheduler jobs appear to be functional.\n");
    else fprintf(stderr,"STATUS: %d agents failed.\n",rc);
    if ((Test > 1) || rc)
	{
	DBclose(DB);
	return(rc);
  ...
  while(KeepRunning)
	...
	/* Got a command! */
	if (Verbose) fprintf(stderr,"Parent got command: %s\n",Input);
	Arg = strchr(Input,'|');
	if (!Arg)
		{
		fprintf(stderr,"ERROR: Unknown command (len=%d) '%s'\n",Len,Input);
	...
	while(!Fed)
	  ...
	  if (Thread >= 0)
		...
		if (Verbose) fprintf(stderr,"(a) Feeding child[%d]: '%s'\n",Thread,Arg);
	  ...
	  /* Thread == -1 is a timeout -- retry the request */
		...
		{
		fprintf(stderr,"ERROR: No living engines for '%s'\n",Input);
  ...
  if (Verbose) fprintf(stderr,"Telling all children: No more food.\n");
  ...
  fprintf(stderr,"Scheduler ending.\n");
