...
class user_edit_any extends Plugin
{
  ...
  function Edit	()
    ...
    if (empty($UserId)) { return("No user selected. No change."); }
    ...
    if (empty($User)) { return("Username must be specified. No change."); }

    /* Make sure password matches */
    if ($Pass1 != $Pass2) { return("Passwords did not match. No change."); }
    ...
    if (substr($R['user_pass'],0,1) == ' ') { $OldBlock = 1; }
    else { $OldBlock = 0; }
  ...
  function Output()
  {
	...
	for($i=0; !empty($Results[$i]['user_pk']); $i++)
	  ...
	  if (substr($R['user_pass'],0,1) == ' ') { $Block = 1; }
	  else { $Block = 0; }
