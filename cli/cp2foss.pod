...
#
# VERSION: $Id: cp2foss.pod 1591 2007-12-17 23:15:16Z markd $
...
General:
cp2foss [-h] {-p <folder-path> -n <folder-name> -a <path-to-archive> -d "description" | [-f <file-of-parameters>]} [-A] [-w]
...
folder specificed by B<-n>, under the folder-path specified with B<-p>.
...
cp2foss will first download the archive using the supplied url using the 
wget utility.  After successful download, the downloaded archive will be 
...
date-base.  Only one upload into a file folder is allowed to avoid
storing dulicate uploads.  The file folder is checked to see if it has been 
...
Switch to indicate that the archive is a url (-a <url>) and should first 
be obtained using wget.
...
The above example, will first get the archive using the url and wget.  It will
