...
class Word2007 extends AbstractWriter implements WriterInterface
{
    ...
    public function save($filename = null)
    {
        ...
        $fileNameOnly = basename($filename,".docx");
	$filename = $this->getTempFile($filename);
        $zip = $this->getZipArchive($filename);
        $phpWord = $this->getPhpWord();
	$this->tempDir = "/tmp/report/$fileNameOnly";
	$this->folderCreate($this->tempDir);
	...
	// Content types
        ...
        foreach ($this->parts as $partName => $fileName) {
	        ...
	        file_put_contents($fileName, $this->getWriterPart($partName)->write());
            }
	}

	$cmd = "cd $this->tempDir && zip -rq report.zip .";
	system($cmd,$ret);
    ...
    private function addHeaderFooterMedia(ZipArchive $zip, $docPart)
    {
        ...
        if (!empty($elements)) {
		    ...
		    $fileName = "$this->tempDir/word/_rels/{$file}.xml.rels";
		    file_put_contents($filename, $writerPart->write());
                }
            }
        }
    }
    ...
    private function addHeaderFooterContent(Section &$section, ZipArchive $zip, $elmType, &$rId)
    {
	    ...
	    $fileName = "$this->tempDir/word/$elmFile";
	    file_put_contents($fileName, $writerPart->write());
        }
    }
    ...
    private function addChart(ZipArchive $zip, &$rId)
    {
		...
		$fileName="$this->tempDir/word/{$filename}";
		file_put_contents($fileName, $writerPart->write());
            }
        }
    }
