 ...
 *****
 * FIX-ME: we should use nftwFileFilter() to eliminate files whose link
 * count is >1; there's no need to parse a potentially-large file more
 * than once.
 */
...
/* */
	...
	*/
	if (isDIR(RAW_DIR)) {			/* temp-unpack dir exists? */
		removeDir(RAW_DIR);	/* get rid of contents now */
	}
	makeDir(RAW_DIR);
	changeDir(RAW_DIR);
