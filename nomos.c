...
static char *progVersion = "NomosVersion: 2.25$";	/* NOT a CVS tag! */
...
int optionIsNotSet(int val)
{
#ifdef	PROC_TRACE
#ifdef	PROC_TRACE_SWITCH
    if (gl.ptswitch)
#endif	/* PROC_TRACE_SWITCH */
	printf("== optionIsNotSet(%x)\n", val);
#endif	/* PROC_TRACE */
	return(!optionIsSet(val));
}
...
static void parseOpts(int argc, char **argv)
{
    ...
    /* CDB 	setOption(OPTS_NOREPORT|OPTS_SRCONLY); */
...
{
    register char *cp, *textp, *md, *what;
    register item_t *ip, *xp;
    register int saveBasename = 1, isPackage = 0, *bufp, notPkgFmt = 0;
    list_t *l;
...
{
	char pathname[myBUFSIZ], *cp;
	register int i, dbMtime, dbIsNewer = 0;
	register list_t *l;
	register item_t *ip;
	double fTime, fFoo;
	item_t *p;
