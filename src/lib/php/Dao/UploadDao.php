...
";
  ...
  private function getUserCycloneDxDefaultsForUpload($uploadId)
  {
    ...
    if (empty($uploadOwner)) {
      return "unchecked,unchecked,unchecked,fossology:,1.7";
    }
    ...
    if (count($settings) < 5) {
      $settings = array_pad($settings, 5, 'unchecked');
    }
    if (empty($settings[3]) || $settings[3] === 'unchecked') {
      $settings[3] = 'fossology:';
    }
    if (empty($settings[4]) || $settings[4] === 'unchecked') {
      $settings[4] = '1.7';
    }
    ...
    $specVersion = $settings[4];

    return "$cyclonedxLicenseComment,$ignoreFilesWOInfo,$osselotExport,$customTagNamespace,$specVersion";
  }
  ...
  public function getCyclonedxSettings($uploadId)
  {
    ...
    // Per-upload settings are stored in comment,ignore,osselot,namespace,specVersion order
