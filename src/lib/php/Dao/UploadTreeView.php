...
class UploadTreeView extends DbViewDao
{
  ...
  private static function getConstraintsCondition(ItemTreeBounds $itemTreeBounds, $constraints)
  {
    ...
    $mapper = function ($constraint) use ($itemTreeBounds)
    {
      return self::getConstraintCondition($itemTreeBounds, $constraint);
    };

    $condition = implode(" AND ", array_map($mapper, $constraints));
