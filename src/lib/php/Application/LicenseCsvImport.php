...
class LicenseCsvImport
{
  ...
  private function updateLicense($row, $rfPk)
  {
    ...
    $extraParams = array();
    $param = array($rfPk);
    if ($row['fullname'] !== null && $row['fullname'] != $oldLicense['rf_fullname']) {
      $param[] = $row['fullname'];
      $stmt .= '.fullN';
    ...
    if ($row['external_id'] !== null && $row['shortname'] !== null && $row['shortname'] != $oldLicense['rf_shortname']) {
      $param[] = $row['shortname'];
      $stmt .= '.shortN';
      $extraParams[] = "rf_shortname=$" . count($param);
      $log .= ", updated shortname";
    }
    if ($row['spdx_id'] !== null && $row['spdx_id'] != $oldLicense['rf_spdx_id']) {
      $param[] = $row['spdx_id'];
      $stmt .= '.spId';
      $extraParams[] = "rf_spdx_id=$" . count($param);
      $log .= ", updated SPDX ID";
    }
    if ($row['text'] !== null && $row['text'] != $oldLicense['rf_text'] && $row['text'] != LicenseMap::TEXT_MAX_CHAR_LIMIT) {
      $param[] = $row['text'];
      $stmt .= '.text';
    ...
    if ($row['url'] !== null && $row['url'] != $oldLicense['rf_url']) {
      $param[] = $row['url'];
      $stmt .= '.url';
      $extraParams[] = "rf_url=$" . count($param);
      $log .= ", updated URL";
    }
    if ($row['notes'] !== null  && $row['notes'] != $oldLicense['rf_notes']) {
      $param[] = $row['notes'];
      $stmt .= '.notes';
      $extraParams[] = "rf_notes=$" . count($param);
      $log .= ", updated notes";
    }
    if ($row['source'] !== null && $row['source'] != $oldLicense['rf_source']) {
      $param[] = $row['source'];
      $stmt .= '.updSource';
      $extraParams[] = "rf_source=$".count($param);
      $log .= ', updated the source';
    }
    if ($row['risk'] !== null && $row['risk'] !== $oldLicense['rf_risk']) {
      $param[] = $row['risk'];
      $stmt .= '.updRisk';
      $extraParams[] = "rf_risk=$".count($param);
      $log .= ', updated the risk level';
    }
    if ($row['licensetype'] !== null && $row['licensetype'] != $oldLicense['rf_licensetype']) {
  ...
  private function handleLicenseDBLicenseImport($row)
  {
    ...
    if (!empty($rfPk)) {
      ...
      $diff = $this->getArrayDiffs($oldObIds, $newObIds);
  ...
  private function getKeyFromShortname($shortname, $groupFk = null)
  {
    ...
    $sql = "SELECT rf_pk FROM ONLY $tableName WHERE rf_shortname = $1 $addCondition AND rf_external_id IS NULL;";
