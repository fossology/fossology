 ...
 *
 * This library provides various methods to handle the FOSSology configuration
 * file.
 */
/* local includes */
...
GRegex* fo_conf_parse;    ///< Regex for parsing
GRegex* fo_conf_replace;  ///< Regex for replace
...
#define BUFFER_SIZE 4096              ///< Maximum buffer length
 ...
 * a list then this uses a system of `"[value1][value2]...[valueN]"` to store
 * the value. This is done under the assumption that lists are not going to be
 * extremely long.
 *
...
/**
 * @brief Load the configuration information from the provided file.
 *
 * If the user has not done a fo_config_free since the last fo_config_load,
 * this will make sure to call that first. In other words, it is assumed that if
 * this is called the configuration file has changed and the user would
 * like to use the new copy.
 *
 * @param rawname The name of the configuration file
 ...
 *
 * @param conf  fo_conf object to load from
 ...
 */
char* fo_config_get(fo_conf* conf, const char* group, const char* key,
    GError** error)
{
  GTree* tree;
  char* ret = NULL;
 ...
 */
char* fo_config_get_list(fo_conf* conf, char* group, char* key, int idx,
    GError** error)
{
  char* val;
  int depth;
...
/**
 * @brief Gets the set of group names.
 *
 * This function returns an array of strings that the user can iterate to get
 * all group names. The user does not own the return of this function and should
 * not free any of the memory associated with it.
 *
 * @param conf   fo_conf object to load from
...
/**
 * @brief Gets the set of key names for a particular group.
 *
 * This returns an array of strings that the user can iterate to get all of the
 * key's for a particular group. This is useful if the keys are not known for a
 * particular group. The array returned by this is owned by the config library
...
/**
 * @brief Checks if the a specific group in the currently parsed configuration
