...
function backToParent($howFar)
{
  ...
  if(is_null($WORKSPACE))
  {
    ...
    if(@chdir($WORKSPACE . "/fossology2.0/fossology/src") === FALSE)
    {
      echo "FATAL! runRUnit could not cd to" . $WORKSPACE . "/fossology2.0/fossology/src\n";
      exit(1);
    }
  }
  ...
  foreach($unitList as $unitTest)
  {
    ...
    if($makeRtn != 0)
    {
      ...
      echo "DB: Restting path back to ...fossology/src\n";
      backToParent('../../..');
      continue;
    }
    else
    {
      ...
      if(chdir('') === FALSE)
      {
      echo "Error! cannot cd to" . $unitTest . "/tests, skipping post processing\n";
      continue;
      }
      */
