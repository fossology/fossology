...
/**
 * @dir
 * @brief Source code of SPDX3 report agent
 * @file
 * @brief SPDX3 report generation
 *
 * Generates reports according to SPDX3 standards.
 * @page spdx3 SPDX3 report
 * @tableofcontents
 * @section spdx3about About SPDX3 agent
 * The agent generates report for an upload according to the spdx3 standard
 * format. It contains
 * -# File path
 * -# Copyright
 ...
 * for every file.
 *
 * @section spdx3actions Supported actions
 * Currently, SPDX3 agent does not support CLI commands and read only from scheduler.
 *
 * @section spdx3source Agent source
 *   - @link src/spdx3/agent @endlink
 *   - @link src/spdx3/ui @endlink
 *   - Functional test cases @link src/spdx3/agent_tests/Functional @endlink
 *   - Unit test cases @link src/spdx3/agent_tests/Unit @endlink
 */

/**
 * @namespace Fossology::SpdxThree
 * @brief Namespace used by SPDX3 agent
 */
namespace Fossology\SpdxThree;
...
include_once(__DIR__ . "/spdx3utils.php");

include_once(__DIR__ . "/version.php");
include_once(__DIR__ . "/services.php");

/**
 * @class SpdxThreeAgent
 * @brief SPDX3 agent
 */
class SpdxThreeAgent extends Agent
{

  const OUTPUT_FORMAT_KEY = "outputFormat";               ///< Argument key for output format
  const DEFAULT_OUTPUT_FORMAT = "spdx3";                  ///< Default output format
  const AVAILABLE_OUTPUT_FORMATS = "spdx3,spdx3json,spdx3rdf";  ///< Output formats available
  ...
  protected function preWorkOnArgs($args)
  {
    if ((!array_key_exists(self::OUTPUT_FORMAT_KEY, $args)
      ...
      $args = SpdxThreeUtils::preWorkOnArgsFlp($args,self::UPLOAD_ADDS,self::OUTPUT_FORMAT_KEY);
    } else {
  ...
  protected function renderPackage($uploadId)
  {
    ...
    $mainLicenseString = [];
    if ($this->outputFormat == "spdx3json" ||
      ...
      $mainLicenseString = SpdxThreeUtils::implodeLicenses(
        SpdxThreeUtils::removeEmptyLicenses($mainLicenseString));
    }
  ...
  protected function toLicensesWithFiles(&$filesWithLicenses, $treeTableName)
  {
    ...
    foreach ($filesWithLicenses as $fileId => $fileNode) {
      ...
      if (! empty($fileNode->getConcludedLicenses())) {
        ...
        $licenses = SpdxThreeUtils::implodeLicenses(
          SpdxThreeUtils::removeEmptyLicenses(array_unique($licenses)));
        ...
        if (! empty($fileNode->getScanners())) {
          ...
          $implodedLicenses = SpdxThreeUtils::implodeLicenses(
            SpdxThreeUtils::removeEmptyLicenses(array_unique($implodedLicenses)));
  ...
  protected function generateFileNodesByFiles($filesWithLicenses, $treeTableName, $uploadId)
  {
    ...
    foreach ($filesWithLicenses as $fileId => $fileData) {
      ...
      if ($this->outputFormat == "spdx3json" ||
        ...
        $concludedLicensesString = SpdxThreeUtils::implodeLicenses(
          SpdxThreeUtils::removeEmptyLicenses($concludedLicensesString));
      }
      if (!$stateWoInfos ||
        ...
        $fileData->setAcknowledgements(
          SpdxThreeUtils::cleanTextArray($fileData->getAcknowledgements()));
        $fileData->setComments(
          SpdxThreeUtils::cleanTextArray($fileData->getComments()));
        $dataTemplate = array(
          'fileId' => $fileId,
          'sha1' => $hashes['sha1'],
          'md5' => $hashes['md5'],
