...
GArray* queryActiveCustomPhrases(fo_dbManager* dbManager) {
  ...
  for (int i = 0; i < numRows; i++) {
    ...
    phrase->licenseIds = queryMappedLicensesForPhrase(dbManager, phrase->cpId);
...
/**
 * \brief Query mapped license IDs for a given phrase
 * \param dbManager Database manager
 * \param cpId Custom phrase ID (cp_pk)
 * \return GArray* of long values (rf_pk)
 */
GArray* queryMappedLicensesForPhrase(fo_dbManager* dbManager, long cpId) {
  PGresult* result = fo_dbManager_ExecPrepared(
    fo_dbManager_PrepareStamement(
      ...
      "SELECT rf_pk FROM custom_phrase_license_map WHERE cp_pk = $1",
      long
    ),
    cpId
  );

  GArray* licenseIds = g_array_new(FALSE, FALSE, sizeof(long));
  
  if (result) {
    int numRows = PQntuples(result);
    for (int i = 0; i < numRows; i++) {
      long rfPk = atol(PQgetvalue(result, i, 0));
      g_array_append_val(licenseIds, rfPk);
    }
    PQclear(result);
  }
...
void phrase_free(Phrase* phrase) {
  ...
  if (phrase->licenseIds) {
    g_array_free(phrase->licenseIds, TRUE);
  }
