...
/**
 * Initial allocation for layers[].  The array grows dynamically via
 * EnsureLayerCap(); MAX_LAYERS_HARD (defined in containerpkg.h) is the
 * absolute ceiling.  This constant is kept here for callers that allocate
 * an initial struct (e.g. test fixtures).
 */
#define MAX_LAYERS  256   ///< Initial layer allocation (soft default — grows dynamically)
...
struct containerpkginfo
{
  ...
  struct containerlayerinfo *layers;  ///< Per-layer metadata array (heap, grows via EnsureLayerCap)
  int   layerCap;                 ///< Allocated capacity of layers[] (0 = not yet allocated)

  /* Multi-manifest support (Docker manifest.json may contain multiple entries) */
  int   manifestCount;            ///< Number of manifest.json array elements processed (0 = unknown)
