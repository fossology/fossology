...
int GetMetadataDocker(long upload_pk, struct containerpkginfo *pi)
{
  ...
  for (int mi = 0; mi < nElems; mi++) {
    ...
    /* cfgBase comes from a JSON field inside the image tarball and must be
     * escaped before interpolation to prevent SQL injection. */
    char *cfgBaseEsc = PQescapeLiteral(db_conn, cfgBase, strlen(cfgBase));
    if (!cfgBaseEsc) {
      LOG_WARNING("containeragent: PQescapeLiteral failed for cfgBase — skipping manifest[%d]\n", mi);
      continue;
    }
    snprintf(SQL, sizeof(SQL),
      "SELECT pfile_sha1||'.'||pfile_md5||'.'||pfile_size"
      ...
      ut, upload_pk, lft, rgt, cfgBaseEsc);
    PQfreemem(cfgBaseEsc);
    result = PQexec(db_conn, SQL);
...
int GetMetadataOCI(long upload_pk, struct containerpkginfo *pi)
{
  ...
  /* manifestHex comes from the OCI index JSON and must be escaped before
   * interpolation to prevent SQL injection. */
  char *manifestHexEsc = PQescapeLiteral(db_conn, manifestHex, strlen(manifestHex));
  if (!manifestHexEsc) {
    LOG_WARNING("containeragent: PQescapeLiteral failed for manifestHex\n");
    free(ut); return -1;
  }
  snprintf(SQL, sizeof(SQL),
    "SELECT pfile_sha1||'.'||pfile_md5||'.'||pfile_size"
    ...
    " WHERE upload_fk=%ld AND lft>%ld AND rgt<%ld"
    "   AND ufile_name=%s LIMIT 1",
    ut, upload_pk, lft, rgt, manifestHexEsc);
  PQfreemem(manifestHexEsc);
  if (PQntuples(result) == 0) {
