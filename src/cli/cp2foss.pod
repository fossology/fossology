...
=head1 SYNOPSIS
 ...
 [-n name] [-d description] [--user user name] [--password password]
...
C<cp2foss --user USER --password PASSWORD
...
C<cp2foss --user USER --password PASSWORD
-d 'regular expression test' '/tmp/*.php'

The above example, B<cp2foss> will upload all the mached php files in tmp dir
