...
void hCopyrightScanner::ScanString(const string& s, list<match>& out) const
{
  ...
  while (pos != end)
  {
    ...
    if (!rx::regex_search(foundPos, end, regException))
    {
      /**
...
void hCopyrightScanner::RemoveNoiseWords(string& text) const{
...
void hCopyrightScanner::StripSuffixes(string& text) const{
  const vector<string> suffixes = {
    "copyright", ".", ",", "year", "parts", "any", "0", "1", "author", "all", "some", "and"
  };
...
void hCopyrightScanner::StripTrailingPeriod(string& text) const{
  if (text.length() < 3 || text.back() != '.') return;

  const vector<string> exceptions = {
    "U.S.A.", "M.I.T.", "e.V.", "e.v.", "b.v.", "S.A.", "inc.", "corp.", "ltd.", "llc.", "co.", "llp."
  };

  for (const auto& ex : exceptions) {
    if (text.size() >= ex.size() &&
        text.compare(text.size() - ex.size(), ex.size(), ex) == 0)
    {
      return;
    }
  }
