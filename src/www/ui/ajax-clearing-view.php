...
class AjaxClearingView extends FO_Plugin
{
  ...
  function Output()
  {
    ...
    switch ($action) {
      ...
      case "saveExpression":
        $this->saveExpression($uploadId, $uploadTreeId, $userId, $groupId, $ast, $expressionId);
        return new JsonResponse();
  ...
  public function getCurrentSelectedLicensesTableData(ItemTreeBounds $itemTreeBounds, $groupId, $orderAscending, $includeExpressions = false, $tableData=true)
  {
    ...
    foreach ($addedClearingResults as $licenseShortName => $clearingResult) {
      ...
      if ($includeExpressions && $clearingResult->getLicenseRef()->getShortName() === 'License Expression') {
        $actionLink = $mainLicenseLink . " <a href=\"javascript:;\" onclick=\"editExpression($licenseId);\"><img src=\"images/button_edit.png\" alt=\"editExpression\" title=\"Edit Expression\" border=\"0\"/></a>";
   ...
   */
  protected function saveExpression($uploadId, $uploadTreeId, $userId, $groupId, $ast, $expressionId=0)
  {
