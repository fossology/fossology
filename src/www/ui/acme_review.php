...
class acme_review extends FO_Plugin
{
  ...
  function RegisterMenus()
  {
    if (GetParm("mod", PARM_STRING) == $this->Name) {
      ...
      if ($detail) {
        ...
        $URI = $this->Name .
          Traceback_parm_keep(array("page", "upload", "folic")) . "&detail=0";
      } else {
        $text = _("ACME Low Level Review");
        $URI = $this->Name .
          Traceback_parm_keep(array("page", "upload", "folic")) . "&detail=1";
      }
  ...
  function FindACMEProjects($uploadtreeRow, &$acme_project_array)
  {
    ...
    if (pg_num_rows($result) > 0) {
      ...
      if (pg_num_rows($projresult) > 0) {
        $acme_project_array[$acme_pfileRow['acme_project_fk']] = pg_fetch_assoc(
          $projresult);
        $acme_project_array[$acme_pfileRow['acme_project_fk']]['count'] = (($uploadtreeRow['rgt'] -
          $uploadtreeRow['lft']) + 1) / 2;
      }
      return;
    } else {
      /* check each child */
  ...
  function HTMLForm($acme_project_array, $upload_pk)
  {
    ...
    foreach ($acme_project_array as $project) {
      /*
       * Set alternating row background color - repeats every $ColorSpanRows
       * rows
       */
      ...
      $ProjectListURL = Traceback_uri() . "?mod=" . $this->Name .
  ...
  function GenerateSPDX($acme_project_array)
  {
    ...
    $spdx .= '      <CreationInfo>' . "\n";
    $spdx .= " <creator>Tool: FOSSology v " . $SysConf['BUILD']['VERSION']
           . " svn " . $SysConf['BUILD']['COMMIT_HASH'] . "</creator>\n";
    ...
    foreach ($acme_project_array as $project) {
      ...
      $spdx .= '<description>' . str_replace("&", " and ",
  ...
  function Output()
  {
    ...
    if (empty($acme_uploadRec)) {
      ...
      $acme_project_array = $this->GetProjectArray1($upload_pk, $nomosAgentpk,
        $MinCount); // low level
      $this->Populate_acme_upload($acme_project_array, $upload_pk, 1);
      $acme_project_array = $this->GetProjectArray0($upload_pk, $nomosAgentpk,
        $MinCount); // high level
      $this->Populate_acme_upload($acme_project_array, $upload_pk, 0);
    }

    $sql = "select * from acme_upload, acme_project "
    ...
    if ($folic) {
      foreach ($acme_project_array as &$project) {
        ...
        while ($acme_pfileRow = pg_fetch_assoc($result)) {
          $LicArray = GetFileLicenses($agent_pk, '',
            $acme_pfileRow['uploadtree_pk'], $uploadtree_tablename);
