...
class AdminLicenseFromCSV extends DefaultPlugin
{
  ...
  protected function handle(Request $request)
  {
    ...
    if ($request->isMethod('POST')) {
      if ($request->get('importFrom') === 'licensedb') {
        ...
        $this->fileLogger->debug("****************** Message From LicenseDB [" . date('Y-m-d H:i:s') . "] ******************");
        $this->fileLogger->debug($vars["message"]);
        $this->fileLogger->debug("****************** End Message From LicenseDB ******************");
      } else {
   ...
   */
  public function handleLicenseDbImport(): string
  {
    $msg = '<br>';
    $data = null;
   ...
   */
  public function checkLicenseDBHealth(): bool
  {

    $getUsers = $this->configuration['url'] . $this->configuration['uri'] . "/health";
    try {
      $response = $this->guzzleClient->get($getUsers);
      if ($response->getStatusCode() === 200) {
        return true;
      }
    } catch (RequestException|GuzzleException $e) {
