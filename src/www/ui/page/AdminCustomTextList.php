...
class AdminCustomTextList extends DefaultPlugin
{
  ...
  private function getTotalBulkCount($searchQuery = '', $userFk = 0)
  {
    ...
    $result = $dbManager->getSingleRow($sql, $params,
      $this->bulkStatementName(__METHOD__, $searchQuery, $userFk));
    return $result ? intval($result['count']) : 0;
  }

  /**
   * The SQL differs per filter, but one name holds only one statement.
   */
  private function bulkStatementName($method, $searchQuery, $userFk)
  {
    return $method . (empty($searchQuery) ? '' : '.search') . ($userFk > 0 ? '.user' : '');
  }
  ...
  private function getBulkDataAjax(Request $request)
  {
    ...
    $result = $dbManager->getRows($sql, $params,
      $this->bulkStatementName(__METHOD__, $searchQuery, $userFk));
    $bulkData = array();
