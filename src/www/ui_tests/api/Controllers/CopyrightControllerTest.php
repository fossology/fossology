...
class CopyrightControllerTest extends \PHPUnit\Framework\TestCase
{
  ...
  public function testGetFileCopyrights()
  {
    ...
    $rows = array(
      'content' => "copyright text", 'hash' => "16f93a4e03cf6b9218fa03ca5a8ee88e",
      'copyright_count' => "1"
    );
    ...
    $this->CopyrightHistPlugin->shouldReceive('getCopyrights')->withArgs([$uploadId, $itemId, $UploadTreeTableName, $agentId, 'statement', 'active', true])->andReturn($res);
    $expectedResponse = (new ResponseHelper())->withJson($res, 200);
    $actualResponse = $this->CopyrightController->getFileCopyrights(null, new ResponseHelper(), ['id' => $uploadId, 'itemId' => $itemId]);
