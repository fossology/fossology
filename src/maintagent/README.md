...
# Maintagent — Quick Usage & When To Use

## Purpose

- `maintagent` is the FOSSology maintenance agent. It runs database and
  repository housekeeping tasks (vacuum/analyze, reindex, remove orphaned
  files/rows, clean logs, remove expired tokens, etc.).

## Behavioral notes

- `maintagent` uses session-local temporary tables and a multi-step approach
  for several operations (notably `removeUploads()`).
  This differs from the older maintagent code which often executed single-statement
  DELETEs with embedded sub-selects.

- Example (what `maintagent` does in `removeUploads()`):
...
- At `agent_verbose >= 3` `maintagent` emits extra timing details via
...
- Check `src/maintagent/maintagent.conf` for module configuration defaults.
...
- maintagent sources: `src/maintagent/agent/` (main, process, utils, usage)
