...
int Prepare_Testing_Data(scheduler_t * scheduler)
{
  ...
  job_pk = 1;
  /* Add the job info */
  sprintf(sql, "INSERT INTO job (job_pk,job_user_fk,job_queued,job_priority,job_name,job_upload_fk) VALUES(%d,'1',now(),'0','testing file',%d)", job_pk,upload_pk);
  database_exec(scheduler, sql);

  jq_pk = 1;
  sprintf(sql, "INSERT INTO jobqueue (jq_pk,jq_job_fk,jq_type,jq_args,jq_runonpfile,jq_starttime,jq_endtime,jq_end_bits,jq_host) VALUES (%d,'%d', 'ununpack', '%d', NULL, NULL, NULL, 0, NULL)", jq_pk, job_pk, upload_pk);
  database_exec(scheduler, sql);
