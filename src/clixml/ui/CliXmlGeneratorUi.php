...
class CliXmlGeneratorUi extends DefaultPlugin
{
  ...
  function preInstall()
  {
    $text = _("Generate CLIXML report");
    menu_insert("Browse-Pfile::Export&nbsp;CLIXML", 0, self::NAME, $text);
    menu_insert("UploadMulti::Generate&nbsp;CLIXML", 0, self::NAME, $text);
  ...
  protected function getJobAndJobqueue($groupId, $upload, $addUploads)
  {
    ...
    $params = array($clixmlAgent->AgentName,$uploadId,$userId,$groupId);
    $log = __METHOD__;
    if ($jqCmdArgs) {
  ...
  /**
   * Schedules CLIXML agent to generate report
   *
   * @param int $groupId
   * @param Upload $upload
